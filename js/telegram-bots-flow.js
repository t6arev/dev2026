(function () {
    ...
    function initVerbRotation() {
        ...
        var words = ['принимает заявки', 'отвечает клиентам', 'оформляет оплату', 'пишет в CRM'];
        var i = 0;
        var swapping = false;
    ...
    function initCasesRotate() {
        var root = document.querySelector('[data-cases-rotate]');
        if (!root) return;

        var cards = Array.from(root.querySelectorAll('[data-case-pair]'));
        if (cards.length < 2) return;

        var pairs = Array.from(new Set(cards.map(function (c) {
            return c.getAttribute('data-case-pair');
        }))).sort();
        ...
        function showPair(pair) {
            cards.forEach(function (card) {
                var on = card.getAttribute('data-case-pair') === pair;
